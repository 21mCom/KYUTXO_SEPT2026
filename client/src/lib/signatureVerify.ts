 ...
 */
export interface FreshnessAnchor {
  height: number;
  hash: string;
  fetchedAt: string;
}
...
export function buildChallengeMessage(params: {
  ...
  const { address, declarantName, declarationDate, purpose, nonce, verifierReference, freshnessAnchor } = params;
  const lines: string[] = [
  ...
  if (verifierReference && verifierReference.trim()) {
    lines.push(`Verifier ref: ${verifierReference.trim()}`);
  }
  if (freshnessAnchor) {
    lines.push(`Block anchor: ${freshnessAnchor.height} / ${freshnessAnchor.hash} (fetched ${freshnessAnchor.fetchedAt})`);
  }
  lines.push(`Address:   ${address}`);
  lines.push('');
  lines.push('Generated by KYUTXO for a Proof of Funds Declaration.');
  return lines.join('\n');
}
