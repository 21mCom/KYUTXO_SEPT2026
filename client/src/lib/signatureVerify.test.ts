...
/**
 * SegWit vectors whose signatures commit to a VERSION-2 to_sign transaction,
 * generated by an INDEPENDENT pure-Python BIP-143 reference-style
 * implementation (hand-written secp256k1 point arithmetic, RFC-6979 ECDSA,
 * BIP-322 to_spend/to_sign construction and bech32 encoding — stdlib only,
 * no bitcoinjs-lib / @noble code). See
 * scripts/proof-vectors/generate_segwit_v2_independent.py. A shared BIP-143
 * sighash bug (e.g. mis-committing nVersion) in the bitcoinjs-derived stack
 * cannot pass both the generator and the verifier here.
 */
// Key: sha256("vault-independent-segwit-v2-vector-key") mod n; pubkey
// 0227b13dfc58407304e395c9b9afc2e3ccfd3ddc87a94336d17388603a9d317e8a
const P2WPKH_V2_INDEP_ADDR = 'bc1q4mp27wnr67u4c8ypwn28peq3jq3wfhfl2e3ehw';
// Witness stack: [DER sig + SIGHASH_ALL, compressed pubkey]
const P2WPKH_V2_INDEP_SIG =
  'AkcwRAIgdzRxtK+ICBL9liDr+d6PMsO4b7NzzUN5986szSrhCvwCIFQKGiPPAfZnYSSBEzPa4wZTaAIePyLGPAVeEjGOpQtFASECJ7E9/FhAcwTjlcm5r8LjzP093IepQzbRc4hgOp0xfoo=';

// P2WSH single-key witness script: <pk> OP_CHECKSIG (same key as above).
const P2WSH_V2_INDEP_ADDR =
  'bc1qyda79uc7p2443kl408zcprag3z56c02nmtyp6zqn8p8ld0l5t75q87dy4c';
// Witness stack: [DER sig + SIGHASH_ALL, witness script]
const P2WSH_V2_INDEP_SIG =
  'AkcwRAIgZYJ0qe1j5ij2/D064HfHAZdtIyJV2VZkWL0u3e1BMUwCIHuyiiDxoooD+WX5XBtZBG4947f0jxfYO7hiH4qb55tBASMhAiexPfxYQHME45XJua/C48z9PdyHqUM20XOIYDqdMX6KrA==';

describe('BIP-322 SegWit version-2 to_sign — independently generated vectors', () => {
  it('verifies an independently generated native P2WPKH v2 proof', async () => {
    const result = await verifyBip322P2WPKH(P2WPKH_V2_INDEP_ADDR, FULL_MSG, P2WPKH_V2_INDEP_SIG);
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('verifies an independently generated P2WSH v2 proof', async () => {
    const result = await verifyBip322Full(P2WSH_V2_INDEP_ADDR, FULL_MSG, P2WSH_V2_INDEP_SIG);
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('routes the independent P2WPKH v2 proof through verifyBitcoinSignature', async () => {
    const result = await verifyBitcoinSignature(P2WPKH_V2_INDEP_ADDR, FULL_MSG, P2WPKH_V2_INDEP_SIG);
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('routes the independent P2WSH v2 proof through verifyBitcoinSignature', async () => {
    const result = await verifyBitcoinSignature(P2WSH_V2_INDEP_ADDR, FULL_MSG, P2WSH_V2_INDEP_SIG);
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('rejects the independent P2WPKH v2 proof against the wrong message', async () => {
    const result = await verifyBip322P2WPKH(
      P2WPKH_V2_INDEP_ADDR,
      'Goodbye World',
      P2WPKH_V2_INDEP_SIG,
    );
    expect(result.verified).toBe(false);
  ...
  it('rejects the independent P2WSH v2 proof against the wrong message', async () => {
  ...
  it('rejects the independent P2WPKH v2 proof against a different P2WPKH address', async () => {
    const result = await verifyBip322P2WPKH(P2WPKH_V2_ADDR, FULL_MSG, P2WPKH_V2_INDEP_SIG);
    expect(result.verified).toBe(false);
  });

  it('rejects the independent P2WSH v2 proof against a different P2WSH address', async () => {
    const result = await verifyBip322Full(P2WSH_V2_ADDR, FULL_MSG, P2WSH_V2_INDEP_SIG);
    expect(result.verified).toBe(false);
  });
});

/**
 * P2SH-wrapped SegWit vectors committing to a VERSION-2 to_sign, generated by
 * the same INDEPENDENT pure-Python implementation (see
 * scripts/proof-vectors/generate_segwit_v2_independent.py). The redeem-script
 * push in the to_sign scriptSig is implied by the wrapper; the proof carries
 * only the witness stack. A shared bug in the bitcoinjs-derived P2SH-nested
 * sighash / redeem-script handling cannot pass both generator and verifier.
 */
// Same key: sha256("vault-independent-segwit-v2-vector-key") mod n.
const P2SH_P2WPKH_V2_INDEP_ADDR = '37Ah4Xr1sx54ac2kCUTdhqoR3hikMNMaeY';
// Witness stack: [DER sig + SIGHASH_ALL, compressed pubkey]
const P2SH_P2WPKH_V2_INDEP_SIG =
  'AkgwRQIhAMeYyxd76UXVlScWH4TJipowg803R3vDfEGwZeeNKPOGAiAO5a3zgQ0elCg5TrQMduVZM4ImtySzh03+BXumaS0tZgEhAiexPfxYQHME45XJua/C48z9PdyHqUM20XOIYDqdMX6K';

// P2SH-P2WSH single-key witness script: <pk> OP_CHECKSIG (same key).
const P2SH_P2WSH_V2_INDEP_ADDR = '3MKDNhpVMLba95nL7pfd48NeMKGEVyJNnR';
// Witness stack: [DER sig + SIGHASH_ALL, witness script]
const P2SH_P2WSH_V2_INDEP_SIG =
  'AkgwRQIhAMogIRwAJoUznfOdsHgWu1mGDITvai+N9rge24S3DJ26AiBh+1FDHG2OidkzWFnkyopEMHnouFvXcER/Q9KnWK9jCgEjIQInsT38WEBzBOOVybmvwuPM/T3ch6lDNtFziGA6nTF+iqw=';

describe('BIP-322 P2SH-wrapped SegWit v2 to_sign — independently generated vectors', () => {
  it('verifies an independently generated P2SH-P2WPKH v2 proof', async () => {
    const result = await verifyBip322P2SH(P2SH_P2WPKH_V2_INDEP_ADDR, FULL_MSG, P2SH_P2WPKH_V2_INDEP_SIG);
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('verifies an independently generated P2SH-P2WSH v2 proof', async () => {
    const result = await verifyBip322P2SH(P2SH_P2WSH_V2_INDEP_ADDR, FULL_MSG, P2SH_P2WSH_V2_INDEP_SIG);
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('routes the independent P2SH-P2WPKH v2 proof through verifyBitcoinSignature', async () => {
    const result = await verifyBitcoinSignature(
      P2SH_P2WPKH_V2_INDEP_ADDR,
      FULL_MSG,
      P2SH_P2WPKH_V2_INDEP_SIG,
    );
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('routes the independent P2SH-P2WSH v2 proof through verifyBitcoinSignature', async () => {
    const result = await verifyBitcoinSignature(
      P2SH_P2WSH_V2_INDEP_ADDR,
      FULL_MSG,
      P2SH_P2WSH_V2_INDEP_SIG,
    );
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('rejects the independent P2SH-P2WPKH v2 proof against the wrong message', async () => {
  ...
  it('rejects the independent P2SH-P2WSH v2 proof against the wrong message', async () => {
    const result = await verifyBip322P2SH(
      P2SH_P2WSH_V2_INDEP_ADDR,
      'Goodbye World',
      P2SH_P2WSH_V2_INDEP_SIG,
  ...
  it('rejects the independent P2SH-P2WPKH v2 proof against a different P2SH address', async () => {
    const result = await verifyBip322P2SH(P2SH_P2WPKH_V2_ADDR, FULL_MSG, P2SH_P2WPKH_V2_INDEP_SIG);
    expect(result.verified).toBe(false);
  });

  it('rejects the independent P2SH-P2WSH v2 proof against a different P2SH address', async () => {
    const result = await verifyBip322P2SH(P2SH_P2WSH_V2_ADDR, FULL_MSG, P2SH_P2WSH_V2_INDEP_SIG);
    expect(result.verified).toBe(false);
  });
});

/**
 * P2SH-P2WSH 2-of-2 OP_CHECKMULTISIG vector committing to a VERSION-2
 * to_sign, generated by the same INDEPENDENT pure-Python implementation (see
 * scripts/proof-vectors/generate_segwit_v2_independent.py). Witness stack:
 * [<empty CHECKMULTISIG dummy>, sig1, sig2, witnessScript], signatures in the
 * same order as the pubkeys inside the script. Exercises the multisig
 * witness-script path (script ordering, empty dummy element) against a
 * generator sharing no code with the bitcoinjs-derived verifier — the existing
 * 2-of-2 / 2-of-3 wrapped-multisig vectors were produced with the same stack
 * the verifier uses, so a shared multisig bug could pass both sides.
 */
// Keys: sha256("vault-independent-segwit-v2-vector-key"[-2]) mod n; pubkeys
// 0227b13dfc58407304e395c9b9afc2e3ccfd3ddc87a94336d17388603a9d317e8a and
// 0292004df0d13cbc1f809bc1a3dac0f95ab3935410f2bc18ab6c69abac2797b9e3.
const P2SH_P2WSH_2OF2_V2_INDEP_ADDR = '3FSur3FAqRSB7GKj12sJVxuagS7aU52tEd';
// Witness stack: [empty dummy, DER sig1 + SIGHASH_ALL, DER sig2 + SIGHASH_ALL, witness script]
const P2SH_P2WSH_2OF2_V2_INDEP_SIG =
  '';

describe('BIP-322 P2SH-P2WSH 2-of-2 multisig v2 to_sign — independently generated vector', () => {
  it('verifies an independently generated 2-of-2 OP_CHECKMULTISIG P2SH-P2WSH v2 proof', async () => {
    const result = await verifyBip322P2SH(
      P2SH_P2WSH_2OF2_V2_INDEP_ADDR,
      FULL_MSG,
      P2SH_P2WSH_2OF2_V2_INDEP_SIG,
    );
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('routes the independent 2-of-2 multisig v2 proof through verifyBitcoinSignature', async () => {
    const result = await verifyBitcoinSignature(
      P2SH_P2WSH_2OF2_V2_INDEP_ADDR,
      FULL_MSG,
      P2SH_P2WSH_2OF2_V2_INDEP_SIG,
  ...
  it('rejects the independent 2-of-2 multisig v2 proof against the wrong message', async () => {
    const result = await verifyBip322P2SH(
      P2SH_P2WSH_2OF2_V2_INDEP_ADDR,
      'Goodbye World',
      P2SH_P2WSH_2OF2_V2_INDEP_SIG,
  ...
  it('rejects the independent 2-of-2 multisig v2 proof against a different P2SH address', async () => {
    const result = await verifyBip322P2SH(
      P2SH_P2WSH_V2_INDEP_ADDR,
      FULL_MSG,
      P2SH_P2WSH_2OF2_V2_INDEP_SIG,
...
/**
 * P2SH-P2WSH 2-of-3 OP_CHECKMULTISIG vector committing to a VERSION-2
 * to_sign, generated by the same INDEPENDENT pure-Python implementation (see
 * scripts/proof-vectors/generate_segwit_v2_independent.py). Only cosigners 1
 * and 3 sign — a NON-ADJACENT subset — so a correct verifier must match
 * signatures to pubkeys in script order while skipping the non-signing middle
 * key. The 2-of-2 vector above (all keys sign) cannot exercise that
 * subset-selection/ordering logic, and the pre-existing 2-of-3 vectors were
 * generated with the same bitcoinjs stack the verifier uses, so a shared
 * subset-selection bug could pass both sides.
 */
// Keys: sha256("vault-independent-segwit-v2-vector-key"[-2|-3]) mod n; pubkey3
// 02149502b624bbbfea177f38138ebbcc9e1f6a9fe596c81b3faf1ddc07fc176f5c.
const P2SH_P2WSH_2OF3_V2_INDEP_ADDR = '3CGz65yzEq732NhSD4VGtwWyZGoodZvk8N';
// Witness stack: [empty dummy, DER sig1 + SIGHASH_ALL, DER sig3 + SIGHASH_ALL, witness script]
const P2SH_P2WSH_2OF3_V2_INDEP_SIG =
  '';

describe('BIP-322 P2SH-P2WSH 2-of-3 multisig v2 to_sign — independently generated vector (cosigners 1 and 3)', () => {
  it('verifies an independently generated 2-of-3 proof signed by a non-adjacent cosigner pair', async () => {
    const result = await verifyBip322P2SH(
      P2SH_P2WSH_2OF3_V2_INDEP_ADDR,
      FULL_MSG,
      P2SH_P2WSH_2OF3_V2_INDEP_SIG,
    );
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('routes the independent 2-of-3 multisig v2 proof through verifyBitcoinSignature', async () => {
    const result = await verifyBitcoinSignature(
      P2SH_P2WSH_2OF3_V2_INDEP_ADDR,
      FULL_MSG,
      P2SH_P2WSH_2OF3_V2_INDEP_SIG,
  ...
  it('rejects the independent 2-of-3 multisig v2 proof against the wrong message', async () => {
    const result = await verifyBip322P2SH(
      P2SH_P2WSH_2OF3_V2_INDEP_ADDR,
      'Goodbye World',
      P2SH_P2WSH_2OF3_V2_INDEP_SIG,
  ...
  it('rejects the independent 2-of-3 multisig v2 proof against a different P2SH address', async () => {
    const result = await verifyBip322P2SH(
      P2SH_P2WSH_2OF2_V2_INDEP_ADDR,
      FULL_MSG,
      P2SH_P2WSH_2OF3_V2_INDEP_SIG,
...
/**
 * INDEPENDENTLY generated Taproot vectors over a VERSION-2 to_sign.
 *
 * The vectors above were produced with the same bitcoinjs-lib +
 * @bitcoinerlab/secp256k1 stack the verifier uses, so a shared BIP-341
 * sighash bug (e.g. mis-committing nVersion) would pass both sides. These
 * two vectors were generated by an INDEPENDENT pure-Python
 * BIP-340/341/342 reference-style implementation (hand-written secp256k1
 * point arithmetic, tagged hashes, taproot tweak, bech32m, BIP-322
 * to_spend/to_sign construction, and the BIP-341 key-path/script-path
...
// Witness stack: [64-byte Schnorr sig, single <pk> OP_CHECKSIG leaf, control block]
const P2TR_SCRIPT_V2_INDEP_SIG =
  'A0AzJpOnjST5SadX0JTBolx4y7jO+F6l1cIFYgxQF8dgsxX1nvgNPLDS4Oee0QIwTXJBmcW0wt3O1UFWBzUgRMslIiBmjKPKP8fzU/58GXGwguTNL/5nDI0EfxsQzQObYcSXnqwhwEgZchMbQkPWMVbRGP4kaIs8EBNAouB0YAIxZWqIC18K';

describe('BIP-322 Taproot version-2 to_sign — independently generated vectors', () => {
  it('verifies an independently generated key-path (Simple) v2 proof', async () => {
    const result = await verifyBip322Simple(
      P2TR_KEYPATH_V2_INDEP_ADDR,
      'Hello World',
      P2TR_KEYPATH_V2_INDEP_SIG,
    );
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('verifies an independently generated script-path (Full) v2 proof', async () => {
    const result = await verifyBip322Full(
      P2TR_SCRIPT_V2_INDEP_ADDR,
      'Hello World',
      P2TR_SCRIPT_V2_INDEP_SIG,
    );
    expect(result.verified).toBe(true);
    expect(result.format).toBe('bip322');
  });

  it('routes the independent key-path v2 proof through verifyBitcoinSignature', async () => {
    const result = await verifyBitcoinSignature(
      P2TR_KEYPATH_V2_INDEP_ADDR,
      'Hello World',
      P2TR_KEYPATH_V2_INDEP_SIG,
  ...
  it('rejects the independent key-path v2 proof against the wrong message', async () => {
    const result = await verifyBip322Simple(
      P2TR_KEYPATH_V2_INDEP_ADDR,
      'Goodbye World',
      P2TR_KEYPATH_V2_INDEP_SIG,
  ...
  it('rejects the independent script-path v2 proof against the wrong message', async () => {
    const result = await verifyBip322Full(
      P2TR_SCRIPT_V2_INDEP_ADDR,
      'Goodbye World',
      P2TR_SCRIPT_V2_INDEP_SIG,
  ...
  it('rejects the independent key-path v2 proof against a different Taproot address', async () => {
    const result = await verifyBip322Simple(
      P2TR_SCRIPT_V2_INDEP_ADDR,
      'Hello World',
      P2TR_KEYPATH_V2_INDEP_SIG,
