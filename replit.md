...
*   **Privacy Audit Interrupted-Notice Guard** (`privacy-audit-interrupted-node-check`, `npx vitest run` on `PrivacyAudit.interruptedNotice.test.tsx`): Covers the three refresh-rehydrate banner paths (mid-audit interrupted, adversary-view interrupted, full restore) by seeding the real privacy-audit session store (fake-indexeddb) and asserting the correct banner — and only that banner — renders on mount via `renderWithProviders`.
*   **Proof of Funds Page Guard** (`pof-page-node-check`, `npx vitest run` on the three `ProofOfFundsDeclaration.*.test.tsx` files: documentIntegrity, freshnessAnchorValidation, nodeUnreachable): Catches regressions in the Proof of Funds page flows (e.g. offline-mode Generate PDF enablement) that previously went unnoticed because these vitest files weren't registered as a validation step.
...
*   **PDF Glyph Rendering Browser Guard** (`pdf-glyph-node-check` vitest + `pdf-glyph-browser-check`, `node scripts/check-pdf-glyph-browser.mjs`): `runPdfGlyphBrowserCheck` (`client/src/lib/pdfGlyphBrowserCheck.ts`) generates a PDF through the real `sanitizePdfText` path and re-parses it with pdf.js to prove remapped WinAnsi glyphs (em-dash, curly quotes, etc.) round-trip in a real viewer. `loadPdfjs()` feature-detects native `Promise.try`: on a modern browser (Chrome 128+) it uses pdf.js's default build + real worker (production configuration); on the Nix-pinned test Chromium (v125) and in Node it falls back to the *legacy* build. Never shim `Promise.try` in the harness — shims don't reach the spawned Web Worker and would fool the detection.
...
*   **Deep-Dive Worker Error Guard** (`deep-dive-worker-error-check`, `npx vitest run` on `transaction-deep-dive.workerError.test.tsx`): Covers the Boltzmann worker's onerror branch (message + Show details + Retry, retry re-runs analysis, next-steps hint after a second worker failure) so a Vite worker-URL bundling regression can't leave users with a silent spinner.
