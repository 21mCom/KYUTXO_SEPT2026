...
*   **Test Provider Harness Guard:** Components that conditionally render an AddressLink/TxidLink throw outside a `RecordPreviewProvider`, so tests must use the shared harness `renderWithProviders`/`TestProviders` (`client/src/test/testProviders.tsx`) rather than a bare inline `<TooltipProvider>`. A registered validation step (`test-providers`, `scripts/check-test-providers.js`, also wired into the pre-commit hook) scans `client/src/**/*.test.tsx?` and flags any test file that renders a `<TooltipProvider>` without also providing a `<RecordPreviewProvider>`. Intentional-mock tests that mock `RecordPreviewContext`/`AddressLink` to assert wiring in isolation (`copy-button-toast.test.tsx`, `copy-button-toast-extra.test.tsx`, `copy-txid-confirmation.test.tsx`) are allow-listed via `ALLOWED_FILES` in the script. Run `node scripts/check-test-providers.js` to verify.
