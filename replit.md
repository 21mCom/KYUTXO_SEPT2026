...
*   **Descriptor Import:** 3-step wizard for importing Bitcoin addresses from output descriptors (Sparrow wallet export format). Parses `wsh(sortedmulti(...))`, `sh(sortedmulti(...))`, and `sh(wsh(sortedmulti(...)))` formats. Extracts M-of-N threshold, xpubs with fingerprints, and script types. Supports dual-chain (external/internal) address derivation with vault metadata attachment. Located at `/descriptor-import`.
