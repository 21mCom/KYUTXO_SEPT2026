...
*   **Statement Report:** Bank-statement-like transaction report generator at `/statement`. Select addresses by pasting or filtering by owner/wallet/tag. Configurable options: date range, BTC/USD currency toggle, three balance display modes (A: no running balance in USD, B: BTC running balance + USD spot value, C: dual columns), optional TXID and address columns. USD prices from historical price database with batch prefetch and ±3 day fallback. PDF export via jsPDF/autotable. **Spending discovery** via `[prevTxid+prevVout]` index finds outgoing transactions even when input participants have zero amounts, with in-memory output amount lookup for fast resolution. Shows incoming/outgoing count badges and warns when all transactions appear positive (potential incomplete Electrs data).
