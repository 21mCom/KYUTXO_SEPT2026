...
*   **Performance:** Utilizes DB-level and cursor-based pagination, indexed lookups (including `inputStringLower` for O(1) case-insensitive record search), and debounced search for efficient data handling. Transactions page uses iterative batched scanning to search across the entire database without caps, yielding between batches to keep the UI responsive. Bulk Editor uses database-level filtering with index-optimized queries instead of loading records into memory, and virtual scrolling (@tanstack/react-virtual) for both the preview list and confirmation dialog to keep the UI smooth with up to 10,000+ matching records. Quick Tagger and Descriptor Import use targeted `inputString` index lookups instead of loading all records.
