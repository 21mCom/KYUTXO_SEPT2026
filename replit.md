...
*   **Security:** Password-based UI lock only (hash check). No field-level encryption — all data is stored as plaintext in IndexedDB. Users needing data-at-rest protection should use OS-level encrypted containers. The `isEncrypted`/`encryptedPayload` fields have been removed from all 16 database tables (DB schema v27). Dead encryption stub files (dbEncryption, encryptedDb, bulk-crypto, key-management) have been deleted. The remaining files under `lib/data/` (record-queries, vocabulary-crud, record-crud) and `dataFacade.ts` contain plain database CRUD operations with no encryption logic.
