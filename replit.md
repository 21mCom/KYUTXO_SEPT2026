...
*   **Performance Optimizations:** DB-level pagination across all major pages (Records, Transactions, Dashboard) to avoid loading entire tables into memory. Participant queries use targeted indexed lookups (`getDecryptedParticipantsByAddresses`/`getDecryptedParticipantsByTxids`) instead of `getAllDecryptedParticipants()` for stats computation, limiting decryption to visible page data only. UTXOs page loads participants only for user-curated addresses with progress indicators.
