...
// vault backup -> restore round-trip when the backup is ENCRYPTED.
//
// The sibling test (entity-list-roundtrip.runtime.test.ts) proves the snapshot
// survives an UNENCRYPTED backup. Encrypted backups take a different path: the
// inline manifest tables (which carry the portable settings preferences,
// including `entityListSnapshot`) are run through the WebCrypto PBKDF2/AES-GCM
// envelope on export and decrypted on restore. A regression in that
// encrypt/decrypt wrapping could silently drop a user's custom entity list for
// encrypted backups only, while the unencrypted guard stays green.
//
...
/**
 * Export the current vault ENCRYPTED, then simulate the snapshot being gone
 * before restore (a fresh device, or a settings-restore regression that wiped
 * it), so restore has to actually re-apply it from the (encrypted) backup
 * rather than the value simply surviving because `settings` is never cleared.
 * Finally restore with the password.
 */
async function exportWipeRestoreEncrypted(): Promise<void> {
  ...
  // Sanity-check that the round-trip really went through the encrypted path.
...
describe("entity-list snapshot encrypted backup round-trip", () => {
  it("restores a replace-mode snapshot with its exact entries and mode", async () => {
    ...
    await exportWipeRestoreEncrypted();
  ...
  it("restores a merge-mode snapshot storing only the user entries + mode", async () => {
    ...
    expect(restored!.entityListSnapshot!.entries[0].address).toBe(
      ADDR.notBundled,
    );
