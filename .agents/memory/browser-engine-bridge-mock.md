...
**Shared scaffolding (2026-08-04):** the common bridge (enable flag, READY status, schema echo, seed no-ops, error envelopes, IDB helpers, records fingerprint) lives in `scripts/engine-bridge-mock.mjs` — `buildEngineBridgeInitScript({ stateFields, helpers, queryHandlers })` returns the init-script string. New engine browser checks must plug per-scope query handlers into it instead of copying the bridge; that's what keeps the checks from drifting apart when the engine IPC surface or fingerprint fields change.
