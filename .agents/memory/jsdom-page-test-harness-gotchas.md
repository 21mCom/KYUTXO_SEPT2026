...
- jsdom's `Element.prototype.scrollIntoView` is missing; opening a real Radix Select rejects from its passive effect unless the test installs a no-op shim before interacting with the trigger.
...
- useSettings-backed toggles: while the settings live-query loads, useSettings returns the DEFAULTS, so checkboxes already show the default state while action buttons are still disabled. Tests must gate on the ENABLED action button (not checkbox state) before clicking, or the click is a silent no-op on cold starts (first test in the file). Pages should disable scan/apply buttons while `isLoading` so a click can't capture the default instead of the persisted choice.
- Adding a SECOND `describe` block to an existing test file: the first describe's `beforeEach`/`afterEach` (clearAllMocks, cleanup, mockResolvedValue) do NOT apply to the new one — mock call counts accumulate across describes and rendered DOM leaks ("multiple elements found", "called 3 times not 1"). Give every new describe its OWN self-sufficient beforeEach (clearAllMocks + re-stub every mock it needs) and afterEach(cleanup).

**Why:** each of these failed loudly with misleading errors while adding AddressChecker component tests (July 2026); the backup/export one surfaced while testing ExportPage's BIP-329 filter export (July 2026); the getter/tables/matchMedia/duplicate-dialog ones surfaced while adding conflict-resolution merge-path page tests (July 2026); the Radix Select shim surfaced while extending UTXO Provenance filter tests (August 2026).
