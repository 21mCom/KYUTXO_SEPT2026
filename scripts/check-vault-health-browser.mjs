...
//
// This script proves three things a mocked/jsdom test cannot:
...
async function main() {
  ...
  console.log(
    `[vault-health-scale] PASSED: Vault Health stays responsive with bounded memory at ${fmt(RECORD_COUNT)} records, cancel works mid-scan, and a superseded refresh never overwrites the newest result.`,
  );
}
