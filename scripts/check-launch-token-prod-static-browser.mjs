...
// Two-tier gate so repeated validation runs don't pay ~20-60s of `vite build`
// each:
//   1. Fast mtime comparison (unchanged behavior): dist/public/index.html
//      newer than every client source / config file → reuse.
//   2. Content-hash build stamp: when sources were merely TOUCHED (mtime bump
//      with identical bytes — common in validation runs), a stamp recording
//      the source-tree content hash + the index.html identity from the last
//      time this check verified/produced a build lets us reuse dist/public
//      without rebuilding. If dist/public was regenerated by anything else
//      (e.g. a real `npm run build`) its index.html mtime no longer matches
//      the stamp, so the stamp is ignored and the normal gate decides.
// The stamp lives in this check's dedicated OUT_DIR; a real `npm run build`
// output under dist/ is never written to or clobbered.
const BUILD_STAMP = path.join(OUT_DIR, 'client-build-stamp.json');
...
// Deterministic sha-256 over the CONTENTS of every input file that shapes
// dist/public (sorted relative paths + bytes), so a touch that changes no
// bytes yields the same hash.
function collectFiles(p, out) {
  ...
  if (st.isDirectory()) {
    for (const entry of fs.readdirSync(p).sort()) {
      if (entry === 'node_modules' || entry.startsWith('.')) continue;
      collectFiles(path.join(p, entry), out);
    }
    return;
  }
  out.push(p);
...
function sourcesContentHash() {
  const files = [];
  collectFiles(path.join(ROOT, 'client'), files);
  collectFiles(path.join(ROOT, 'vite.config.ts'), files);
  collectFiles(path.join(ROOT, 'package.json'), files);
...
function indexHtmlStat() {
  return fs.statSync(path.join(DIST_PUBLIC, 'index.html'), { throwIfNoEntry: false });
}

function readBuildStamp() {
  try {
    const stamp = JSON.parse(fs.readFileSync(BUILD_STAMP, 'utf8'));
    if (stamp && typeof stamp.sourcesHash === 'string' && typeof stamp.indexHtmlMtimeMs === 'number') {
      return stamp;
    }
  } catch {}
  return null;
...
function writeBuildStamp(sourcesHash) {
  const st = indexHtmlStat();
  if (!st) return;
  // OUT_DIR is recreated by buildProdServerBundle() later in this run; the
  // stamp is (re)written after that in main(). Here we just ensure the dir.
  fs.mkdirSync(OUT_DIR, { recursive: true });
  fs.writeFileSync(
    BUILD_STAMP,
    JSON.stringify({ sourcesHash, indexHtmlMtimeMs: st.mtimeMs, indexHtmlSize: st.size }),
  );
}

function distPublicIsFreshByMtime() {
  const st = indexHtmlStat();
  if (!st) return false;
...
function distPublicMatchesStamp(sourcesHash) {
  const st = indexHtmlStat();
  if (!st) return false;
  const stamp = readBuildStamp();
  if (!stamp) return false;
  // The stamp only vouches for the exact dist/public it recorded. If anything
  // rebuilt dist/public since (mtime/size mismatch), ignore the stamp.
  if (stamp.indexHtmlMtimeMs !== st.mtimeMs) return false;
  if (typeof stamp.indexHtmlSize === 'number' && stamp.indexHtmlSize !== st.size) return false;
  return stamp.sourcesHash === sourcesHash;
}

// Returns the sources content hash so main() can re-stamp after OUT_DIR is
// recreated.
function ensureClientBuild() {
  if (distPublicIsFreshByMtime()) {
    console.log(`[${LABEL}] reusing fresh dist/public (newer than client sources)`);
    return sourcesContentHash();
  }
  const sourcesHash = sourcesContentHash();
  if (distPublicMatchesStamp(sourcesHash)) {
    console.log(
      `[${LABEL}] reusing dist/public (sources touched but content-hash unchanged since last verified build)`,
    );
    return sourcesHash;
  }
...
async function main() {
  ...
  const sourcesHash = ensureClientBuild();
  buildProdServerBundle();
  // buildProdServerBundle() recreates OUT_DIR, so stamp after it: record the
  // verified/produced dist/public against the current source content hash so
  // later runs skip the rebuild when sources are touched but unchanged.
  writeBuildStamp(sourcesHash);
