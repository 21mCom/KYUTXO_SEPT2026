...
function findPortableArtifact() {
  ...
  if (fs.existsSync(expected) && fs.statSync(expected).size > 0) {
    const portableMtime = fs.statSync(expected).mtimeMs;
    const asarMtime = fs.statSync(ASAR).mtimeMs;
    if (portableMtime + 1_000 < asarMtime) {
      throw new Error(
        `${TAG} generated portable artifact predates the validated app.asar: ${expected}`,
      );
    }
    return expected;
  }
