...
//
// WHY: completion validation runs many scripts/check-*-browser*.mjs checks in
...
//
// Rule: every file matching scripts/check-*-browser*.mjs must contain BOTH:
...
const files = fs
  ...
  .filter((f) => /^check-.*-browser.*\.mjs$/.test(f))
  .sort();

if (files.length === 0) {
  console.error(
    'check-browser-check-lock: no scripts/check-*-browser*.mjs files found — glob or layout changed?',
  );
  process.exit(1);
}
