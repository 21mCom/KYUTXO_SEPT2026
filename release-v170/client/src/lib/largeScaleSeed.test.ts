...
// The repository is a process-level singleton in production.  Keep this
// fixture faithful to the database mock while rotating the TestDb per test;
// otherwise the next test reuses the closed Dexie instance from afterEach.
vi.mock("@/lib/repository", async () => {
  ...
  return {
    ...actual,
    getVaultRepository: () => new actual.DexieVaultRepository(testDb as any),
  };
});
