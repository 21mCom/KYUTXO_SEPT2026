...
describe("computeMultiHopKnown", () => {
  beforeAll(async () => {
    // getVaultRepository caches the Dexie adapter for the lifetime of the
    // module. Keep one fixture database alive across cases so the adapter
    // cannot retain a closed database after an individual test's cleanup.
    testDb = new TestDb(`test-multihop-${Date.now()}-${Math.random()}`);
    await testDb.open();
  });
