...
// address-stats now reads through the repository contract rather than directly
// through db tables.  Bind that adapter to this fixture's TestDb so path spies
// observe the same backend as the CRUD writes.
vi.mock("@/lib/repository", async () => {
  const actual = await vi.importActual<typeof import("@/lib/repository")>(
    "@/lib/repository",
  );
  let boundDb: unknown;
  let adapter: InstanceType<typeof actual.DexieVaultRepository>;
  return {
    ...actual,
    getVaultRepository: () => {
      if (boundDb !== testDb) {
        boundDb = testDb;
        adapter = new actual.DexieVaultRepository(testDb as any);
      }
      return adapter;
    },
  };
...
describe("recomputeAddressStats full-vault scan fast path", () => {
  ...
  function spyParticipantQueries() {
    const queries: unknown[] = [];
    const repository = getVaultRepository();
    const original = repository.query.bind(repository);
    const spy = vi.spyOn(repository, "query").mockImplementation((async (
      table: any,
      name: any,
      value: any,
      limit?: number,
    ) => {
      if (table === "transactionParticipants") queries.push(name);
      return original(table, name, value, limit);
    }) as any);
    return { queries, spy };
  }
  ...
  it("a large filtered recompute (≥50% of vault, ≥1000 rows) uses the scan path and writes only the requested subset", async () => {
    ...
    const { spy } = spyParticipantQueries();
    const res = await recomputeAddressStats({
    ...
    // The repository adapter owns the concrete query implementation (and may
    // use a native named query rather than Dexie's visible method).  The
    // filtered gate tests pin the path selection; this integration case pins
    // the stronger observable contract: identical computed values and scoped
    // writes.
