...
// Keep the repository adapter aligned with the per-file TestDb.  The
// production repository is intentionally cached, but a test rotates its
...
beforeEach(async () => {
  ...
  // The scan is implemented behind the repository contract; spying on the
  // table's legacy method misses calls when the adapter is swapped.
  participantCountSpy = vi.spyOn(getVaultRepository(), "count");
});
...
const scanRan = () =>
  participantCountSpy.mock.calls.some(([table]) => table === "transactionParticipants");
