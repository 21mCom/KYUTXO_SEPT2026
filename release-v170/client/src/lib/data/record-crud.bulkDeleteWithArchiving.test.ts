...
class TestDb extends Dexie {
  ...
  recordOrigins!: Table<{ id?: number; recordId: number }, number>;
  constructor(name: string) {
    super(name);
    this.version(1).stores({
      records: "++id, type, inputString, inputStringLower, updatedAt",
...
beforeEach(async () => {
  ...
  deleteSpy.mockClear();
  dbChangeEvents = [];
  if (!testDb) testDb = new TestDb(`KYUTXO-bulk-delete-${Date.now()}-${Math.random()}`);
  else await Promise.all(testDb.tables.map((table) => table.clear()));
  vi.stubGlobal(
    "fetch",
    vi.fn(async () => {
