...
function formatDeviceValue(key: string, settings: any): string | null {
  switch (key) {
    ...
    case "categoryMappingDraft":
      return settings.categoryMappingDraft &&
        typeof settings.categoryMappingDraft === "object"
        ? `${Object.keys(settings.categoryMappingDraft).length} decision(s)`
        : null;
    case "categoryMappingCheckpoints":
      return settings.categoryMappingCheckpoints &&
        typeof settings.categoryMappingCheckpoints === "object"
        ? `${Object.keys(settings.categoryMappingCheckpoints).length} checkpoint(s)`
        : null;
    default:
      return null;
  }
  ...
  it("applies every pref when the backup carries usable values for all of them", async () => {
    ...
    const backupRow = {
      ...
      categoryMappingDraft: {
        "legacy-category": { kind: "rename", categoryName: "Current category" },
      },
      categoryMappingCheckpoints: {
        "legacy-category": { status: "applied", appliedAt: 1 },
      },
    };
