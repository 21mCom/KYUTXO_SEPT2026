// End-to-end guard that a NEW-format (v3) backup's evidence FILES actually open
// after restore — not just that the database links are correct.
//
// Task #541 proved this for the LEGACY (pre-v3) restore path
// (`legacy-evidence-files.runtime.test.ts`). The current v3 streaming restore
// performs the SAME evidenceId remap, but in a different place:
// `restoreInlineTables` (`inline-tables.ts`) re-`add`s evidence rows so they
// receive FRESH autoincrement ids (clear() does NOT reset IndexedDB key
// generation), then relinks every attachment's `evidenceId` to the new id. That
// remap is only exercised by `inline-tables-roundtrip.runtime.test.ts`, which
// runs over fake-indexeddb only — it proves the DB pointer is right but NOT that
// the pointer resolves to a real, readable file on disk. A correct link that
// points at a missing or wrong file would still pass that test while the user
// sees a broken document.
//
// This test closes that gap for the v3 path. It wires up the REAL pieces:
...
//   - the REAL v3 `restoreInlineTables` restore path over fake-indexeddb.
// It then restores a v3 backup's inline evidence whose ids SHIFT on restore (the
...
let restoreInlineTables: typeof import("./inline-tables").restoreInlineTables;
...
beforeAll(async () => {
  tmpDataDir = await fs.mkdtemp(path.join(os.tmpdir(), "kyutxo-evidence-v3-"));
  ...
  restoreInlineTables = (await import("./inline-tables")).restoreInlineTables;
...
describe("v3 restore: evidence files open via their restored path", () => {
  it("each restored attachment fetches the file belonging to the SAME evidence after ids shift", async () => {
    ...
    // 3. Build the v3 backup's inline-table payload. Evidence carry backup ids
    //    1/2/3; attachments reference those backup ids (one per evidence).
    const backupData = {
      evidence: plan.map((p) => ({
        id: p.backupId,
        title: p.title,
      ...
      evidenceAttachments: plan.map((p) => ({
        id: p.backupId * 10,
    ...
    // 4. Run the REAL v3 inline-table restore path.
    await restoreInlineTables(backupData);
    ...
    const restored = allEvidence.filter(
      (e) =>
        e.title.startsWith("Coinbase") ||
        e.title.startsWith("Bank") ||
        e.title.startsWith("Cold"),
    );
