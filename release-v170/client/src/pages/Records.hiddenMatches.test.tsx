...
vi.mock("@/lib/database", async () => {
  ...
  return {
    ...
    db: {
      // DexieVaultRepository prefers Dexie's dynamic table(name) helper when
      // present. Keep this fixture faithful to both supported database shapes
      // so repository-backed page reads exercise the same table contract.
      table: (name: string) => ({ records, blockchainTransactions: {
        where: () => ({
          startsWithIgnoreCase: () => ({ limit: () => ({ toArray: () => Promise.resolve([]) }) }),
        }),
      }, customFields: {
      ...
      } } as Record<string, unknown>)[name] ?? records,
      records,
