...
// Privacy Audit report panel (Reports.tsx). The panel lets users:
//   - click a Score Breakdown (waterfall) row to highlight + scroll to its
//     matching finding rows (breakdown -> findings), and
//   - click a finding row to highlight + scroll to its matching waterfall row
//     (findings -> breakdown, the reverse direction).
// It also shows each finding's rounded score impact (scoreDelta) inline.
//
...
describe("PrivacyAuditReportPanel finding <-> breakdown navigation", () => {
  it("clicking a finding row highlights its matching waterfall row", async () => {
    ...
    // Click the first finding row (ADDRESS_REUSE).
  ...
  it("clicking a waterfall row highlights its matching finding row (reverse)", async () => {
    const { getByTestId, container } = await renderWithResult();

    // Waterfall rows are indexed 0=base,1=ADDRESS_REUSE,2=DUST.
    fireEvent.click(getByTestId("row-privacy-waterfall-2"));

    // The matching finding row (DUST -> finding index 1) is highlighted.
    const dustFinding = container.querySelector<HTMLElement>(
      '[data-finding-type="DUST"]',
    );
    expect(dustFinding).not.toBeNull();
    expect(dustFinding!.className).toContain("bg-muted");

    // The other finding row is not highlighted.
    ...
    // And scrollIntoView fired on the targeted finding row.
    expect(dustFinding!.scrollIntoView).toHaveBeenCalled();
  });

  it("clicking the base waterfall row does nothing (no findings to jump to)", async () => {
    const { getByTestId, container } = await renderWithResult();

    fireEvent.click(getByTestId("row-privacy-waterfall-0"));

    expect(container.querySelector(".bg-muted")).toBeNull();
  });

  it("shows each finding's rounded score impact", async () => {
    const { getByTestId } = await renderWithResult();

    // scoreDelta -12.4 rounds to -12 pts.
    expect(getByTestId("text-privacy-finding-impact-0").textContent).toContain("-12 pts");
    // scoreDelta -2.6 rounds to -3 pts.
