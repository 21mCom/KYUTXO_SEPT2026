...
import { renderSourceNote } from "./PrivacyAudit";
...
describe("renderSourceNote", () => {
  ...
  it("keeps trailing punctuation out of the link target but renders it as text", () => {
    ...
    expect(link.getAttribute("href")).toBe("https://example.com");
    expect(link.textContent).toBe("https://example.com");
    // The trailing period stays as plain text after the link.
    expect(container.textContent).toBe("see https://example.com.");
  });

  it("handles a URL wrapped in parentheses without swallowing the closing paren", () => {
    const { getByRole, container } = render(
      <>{renderSourceNote("(https://example.com)")}</>,
    );
    const link = getByRole("link") as HTMLAnchorElement;
    expect(link.getAttribute("href")).toBe("https://example.com");
    expect(link.textContent).toBe("https://example.com");
    // The closing paren is preserved as text outside the link.
  ...
  it("opens links via window.open on click and prevents default navigation", () => {
    ...
    // Default navigation is prevented (event.preventDefault was called).
  ...
  it("does not perform any network request at render time", () => {
    ...
    // Rendering must never fetch or open anything on its own.
