...
- The EXT_* "external independent vectors" (P2WSH 2-of-2/2-of-3 multisig, P2TR leaf/CSA/multi-leaf) are now covered too: original throwaway keys were unrecoverable (seed brute-force failed), so the vectors were REGENERATED from documented seeds (`"kyutxo ext p2wsh cosigner A/B/C"`, `"kyutxo ext taproot internal/leaf key"`, `"kyutxo ext taproot csa key A/B"`, `"kyutxo ext taproot tree leaf key 0..3"`) via gen_ext_* in the main generator; version-0 to_sign, EXT_MSG, stack shapes preserved (2-of-3 signed by non-adjacent A+C; CSA stack [sigB, sigA, leaf, control]).
