...
**Also:** `stream/promises.pipeline` into an fs WriteStream created with `autoClose: false` hangs forever (no 'close' event ever fires). Let each stream own its FileHandle (`autoClose: true`) and skip manual `fh.close()`.
