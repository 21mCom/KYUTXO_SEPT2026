...
- npm audit emits VALID-JSON error bodies (`{"error":{"code":...}}`, no `vulnerabilities` map) on registry/network/auth failure — any audit gate must hard-fail on `error` or a missing `vulnerabilities` object, or an unreachable registry silently passes as a clean tree.
